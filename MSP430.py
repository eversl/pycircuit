...
@author: leonevers
'''
from PyCircuit import Vector, Signal, Memory, Decoder, FlipFlops, FeedbackVector, If, zip_all, FeedbackSignal, Or, calcAreaDelay, \
    Enum, Case, EnumVector, And, KoggeStoneAdder, Negate, GuardedMemory
...
def decodeInstr(word):
    inst_type = If(word[15] | (~word[15] & word[14]), I_TYPES['IT_TWO'],
                   If(word[13], I_TYPES['IT_JUMP'], I_TYPES['IT_TWO']))
    instr = EnumVector(INSTRS, If(word[15] | (~word[15] & word[14]), Vector(word[12:]),
                                  If(word[13], Vector(word[10:13] + [Signal()]), Vector(word[7:10] + [Signal()]))))
...
def alu(instr, src, dst_in, flags_in, bw):
    ...
    def do_add_sub(als, bls, c, instr):
        ...
        n_out = sls[-1]  # negative equals sign bit
        z_out = And(* (~(a ^ b) for a, b in zip_all(Vector(sls), Vector(0, len(als)))))
        v_out = (als[-1] ^ sls[-1]) & (bls[-1] ^ sls[-1])
        return Vector(sls), {'c': c_out,
                     'n': n_out,
                     'z': z_out,
                     'v': v_out}
    ...
    def op(src, dst_in):
        c_in = flags_in['c']
        dst_out, flags_out = Case(instr, {INSTRS['MOV']: (src, flags_in),
                                          (INSTRS['ADD'], INSTRS['ADDC'], INSTRS['SUB'], INSTRS['SUBC'], INSTRS['CMP']):
                                          do_add_sub(src, dst_in,
                                                    Case(instr, {(INSTRS['ADD'],  INSTRS['SUB'], INSTRS['CMP']): Signal(),
                                          ...
                                          INSTRS['XOR']: (src ^ dst_in, flags_in),  # TODO: set flags
    ...
    dst_out, flags_out = Case(bw, {
            BYTE_WORD['BYTE']: (lambda dst_out, flags_out: (Vector(dst_out[:8] + dst_in[8:]), flags_out))(*op(Vector(src[:8]), Vector(dst_in[:8]))),
...
def CPU(mem_init, clk):
    ...
    mem_out = FeedbackVector(0,16)

    current_instr_word = FeedbackVector(0, 16)

    instr_word = Case(state, {STATES['FETCH']: mem_out}, default=current_instr_word)
    current_instr_word.connect(FlipFlops(instr_word, clk))

    instr, inst_type, src_reg, dst_reg, inst_bw, a_s, a_d, offset = decodeInstr(instr_word)


    src_out, dst_out, regs = RegisterFile(pc_incr = Case(state, {STATES['FETCH']: Signal(True),
                                                                 STATES['SRC_IDX']: Signal(True),
                                                                 STATES['DST_IDX']: Signal(True)}),
                                          src_reg = src_reg,
                                          dst_reg = dst_reg,
                                          src_incr = Case(state, {STATES['SRC_INCR']: Signal(True)}),
                                          dst_in = Case(bw, {BYTE_WORD['BYTE']: Vector(dst_in[:8] + [Signal()] * 8),
    ...
    mem_in = Case(bw, {BYTE_WORD['BYTE']: If(mem_addr[0], Vector(dst_in[8:] + dst_in[:8]), dst_in),
                       BYTE_WORD['WORD']: dst_in})
    mem_out.connect(Memory(mem_addr[1:8], mem_in,
    ...
    alu_out, flags_out = alu(instr, src, dst, flags_in, bw)
    ...
    return {'state': state, 'src_reg': src_reg, 'dst_reg': dst_reg, 'bw': bw,
            ...
            'instr': instr, 'sr_in': sr_in, 'do_alu': do_alu,}
