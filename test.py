...
from PyCircuit import TestSignal, FullAdder, SRLatch, DLatch, DFlipFlop, \
    ...
    Decoder, Memory, ROM, SyntheticMemory, calcAreaDelay, KoggeStoneAdder, \
    TestVector, DecimalAdder, simplify, Case, DontCare, current_cache, Clock, Register, ConstantSignal, \
    SyntheticAdder, SyntheticSubtractor, SyntheticMultiplier, SyntheticDecimalAdder, ConstantVector
...
class Test(unittest.TestCase):
    ...
    def assertCurrent(self, vec):
        current_cache.clear()
        self.assertTrue(vec.current() == signalsToInt(vec.ls, False) or
                        (vec.current() & ((1 << len(vec)) - 1)) == signalsToInt(vec.ls, False))
    ...
    def test_DecimalAdder(self):
        for a in xrange(0, 10000, 907):
            ...
            for b in xrange(0, 10000, 359):
                ...
                for c in xrange(2):
                    sls = DecimalAdder(d_a, d_b, ConstantVector(c, 1))
                    sls, c_out = sls[:-1], sls[-1]
                    self.assertCurrent(sls)
                    d_sum = sls.concat(c_out)
                    sum = int(''.join(reversed([str(d_sum[i:i + 4].toUint()) for i in xrange(0, len(d_sum), 4)])))
    ...
    def test_SyntheticMemory(self):
        clk = Clock()
        SZ = 10
        a = TestVector(0, SZ)
        d = TestVector(0, 16)
        mem_wr = TestVector([False])
        q = SyntheticMemory(clk, a, d, mem_wr)
        self.assertCurrent(q)
        print calcAreaDelay(a.concat(d).concat(mem_wr))
        for i in xrange(2 ** SZ):
            a[:] = i
    ...
    def test_SyntheticArith(self):
        als = TestVector(0, 16)
        bls = TestVector(0, 16)
        cin = TestVector(0, 1)
        addls = SyntheticAdder(als, bls, cin)
        subls = SyntheticSubtractor(als, bls, cin)
        mulls = SyntheticMultiplier(als, bls)
        for a in xrange(-256, 255, 67):
            for b in xrange(-22756, 32767, 1453):
                for c in xrange(2):
                    als[:] = a
    ...
    def test_SyntheticDecimalAdder(self):
        als = TestVector(0, 16)
        bls = TestVector(0, 16)
        cin = TestVector(0, 1)
        addls = SyntheticDecimalAdder(als, bls, cin)
        for a in xrange(0, 10000, 907):
            for b in xrange(0, 10000, 359):
                for c in xrange(2):
                    als[:] = int("%04u" % a, 16)
